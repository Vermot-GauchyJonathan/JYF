...
import roadTextureUrl from "../assets/textures/course.jpg"; // Changed texture
...
class Game {
  ...
  updateMoves(delta) {
    if (this.inputMap["KeyA"]) {
      ...
      if (this.player.position.x < -3.75) this.player.position.x = -3.75;
  ...
  async createScene() {
    ...
    this.scene.clearColor = new Color3(0.5, 0.5, 0.5); // Gris
    ...
    var pipeline = new DefaultRenderingPipeline("default", true, this.scene, [
      this.camera,
    ]);
    ...
    this.playerBox = MeshBuilder.CreateCapsule("playerCap", {
      width: 0.4,
      height: 1.7,
    });
    ...
    let mainTrack = MeshBuilder.CreateBox("trackmiddle", {
      width: TRACK_WIDTH,
      height: TRACK_HEIGHT,
      depth: TRACK_DEPTH,
    });
    ...
    this.music = new Sound("music", musicUrl, this.scene, undefined, {
      loop: true,
      autoplay: true,
      volume: 0.4,
    });
